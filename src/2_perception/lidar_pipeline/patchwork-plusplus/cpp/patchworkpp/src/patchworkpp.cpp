...
void PatchWorkpp::estimate_plane(const vector<PointXYZ> &ground) {
    ...
    if (ground.empty()) {
        cout << "PatchWorkpp::estimate_plane() - No ground points to estimate the plane!" << endl;
        return;
    }
...
void PatchWorkpp::extract_piecewiseground(
    ...
    if (params_.enable_RVPF)
    {
        for (int i = 0; i < params_.num_iter; i++)
        {
            ...
            std::cout << "zone_idx: " << zone_idx << ", normal_ size: " << normal_.size() << std::endl;
            // if normal is empty, fill it with the previous normal
