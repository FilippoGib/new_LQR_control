...
bool Way::segmentsIntersect(const local_planner::Point &A, const local_planner::Point &B, const local_planner::Point &C, const local_planner::Point &D) {
  return local_planner::Point::ccw(A, C, D) != local_planner::Point::ccw(B, C, D) and local_planner::Point::ccw(A, B, C) != local_planner::Point::ccw(A, B, D);
}
...
bool Way::closesLoop() const {
  return this->size() >= MIN_LOOP_SIZE and local_planner::Point::distSq(this->front().midPoint(), this->back().midPoint()) <= params_.max_dist_loop_closure * params_.max_dist_loop_closure;
}

bool Way::closesLoopWith(const Edge &e, const local_planner::Point *lastPosInTrace) const {
  local_planner::Point actPos = lastPosInTrace ? *lastPosInTrace : this->back().midPoint();
  return
...
std::vector<local_planner::Point> Way::getPath() const {
...
Tracklimits Way::getTracklimits() const {
  ...
  local_planner::Point pAnt = this->empty() ? local_planner::Point(0, 0) : local_planner::Point(-50, this->front().midPointGlobal().y);  // This only works in a global, the -50 is arbitrary
