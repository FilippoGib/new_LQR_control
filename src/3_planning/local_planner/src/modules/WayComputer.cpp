...
void WayComputer::filterMidpoints(EdgeSet &edges, const TriangleSet &triangulation) const {
  ...
  std::vector<local_planner::Point> circums(triangulation.size());
  std::transform(triangulation.begin(), triangulation.end(), circums.begin(),
                 [](const Triangle &t) -> const local_planner::Point & { return t.circumCenter(); });
  KDTree circumKDTree(circums);

  // Perform the filtering
  auto it = edges.begin();
  while (it != edges.end()) {
    local_planner::Point midPoint = it->midPoint();
    KDTData<size_t> nearestCC = circumKDTree.nearest_index(midPoint);
    if (bool(nearestCC) and local_planner::Point::distSq(circums[*nearestCC], midPoint) > pow(this->params_.max_dist_circum_midPoint, 2)) {
      it = edges.erase(it);
    } else {
      it++;
    }
...
double WayComputer::getHeuristic(const local_planner::Point &actPos, const local_planner::Point &nextPos, const Vector &dir, const Params::WayComputer::Search &params) const {
  double distHeur = local_planner::Point::dist(actPos, nextPos);
...
void WayComputer::findNextEdges(std::vector<HeurInd> &nextEdges, const Trace *actTrace, const KDTree &midpointsKDT, const std::vector<Edge> &edges, const Params::WayComputer::Search &params) const {
  ...
  local_planner::Point actPos(0, 0);
  local_planner::Point lastPos(0, 0);
...
size_t WayComputer::treeSearch(std::vector<HeurInd> &nextEdges, const KDTree &midpointsKDT, const std::vector<Edge> &edges, const Params::WayComputer::Search &params) const {
  ...
  while (not cua.empty()) {
    ...
    if (trace_at_max_height or nextEdges.empty()) {
      ...
      for (const HeurInd &nextEdge : nextEdges) {
        local_planner::Point actPos = edges[t.edgeInd()].midPoint();
...
void WayComputer::computeWay(const std::vector<Edge> &edges, const Params::WayComputer::Search &params) {
  ...
  std::vector<local_planner::Point> midpoints(edges.size());
  std::transform(edges.begin(), edges.end(), midpoints.begin(),
                 [](const Edge &e) -> local_planner::Point { return e.midPoint(); });
...
std::vector<local_planner::Point> WayComputer::getPath() const {
...
nav_msgs::msg::Path WayComputer::getPathCenterLine() const {
  nav_msgs::msg::Path res;
  res.header.frame_id = "track"; // Set the frame of reference
  res.header.stamp = this->lastStamp_;

  // Extract points from wayToPublish_
  std::vector<local_planner::Point> path = this->wayToPublish_.getPath();

  for (const local_planner::Point &p : path) {
    geometry_msgs::msg::PoseStamped poseStamped;
    poseStamped.header = res.header; // Use the same header
    poseStamped.pose.position = p.gmPoint(); // Convert Point to ROS Point
    poseStamped.pose.orientation.w = 1.0; // Default orientation
    poseStamped.pose.orientation.x = 0.0;
    poseStamped.pose.orientation.y = 0.0;
    poseStamped.pose.orientation.z = 0.0;
    
    res.poses.push_back(poseStamped); // Append to path
  }
